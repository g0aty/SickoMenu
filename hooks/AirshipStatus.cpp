...
{
	if (State.ShowHookLogs) Log.HookDebug("Hook dAirshipStatus_OnEnable executed", false);
...
float dAirshipStatus_CalculateLightRadius(AirshipStatus* __this, NetworkedPlayerInfo* player, MethodInfo* method) {
	if (State.ShowHookLogs) Log.HookDebug("Hook dAirshipStatus_CalculateLightRadius executed", false);
