...
{
	if (State.ShowHookLogs) Log.HookDebug("Hook dSaveManager_GetPurchase executed", false);
	return true;
}

// v2022.10.25s
bool dPlayerPurchasesData_GetPurchase(PlayerPurchasesData* __this, String* itemKey, String* bundleKey, MethodInfo* method) {
	if (State.ShowHookLogs) Log.HookDebug("Hook dPlayerPurchasesData_GetPurchase executed", false);
