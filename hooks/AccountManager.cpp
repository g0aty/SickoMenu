...
void dAccountManager_UpdateKidAccountDisplay(AccountManager* __this, MethodInfo* method) {
    if (State.ShowHookLogs) Log.HookDebug("Hook dAccountManager_UpdateKidAccountDisplay executed", false);
    // grant permissions
    if (!State.PanicMode) {
...
bool dAccountManager_CanPlayOnline(AccountManager* __this, MethodInfo* method) {
    if (State.ShowHookLogs) Log.HookDebug("Hook dAccountManager_CanPlayOnline executed", false);
    return true;
}