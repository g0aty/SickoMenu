...
enum Groups { General, Spoofing, Keybinds };
...
void Render() {
  ...
  ImGui::BeginChild("###Settings", ImVec2(500 * State.dpiScale, 0), true,
  ...
  if (openGeneral) {
    ...
    ImGui::SameLine();
    if (ToggleButton("Allow Activating Keybinds while Chatting",
                     &State.KeybindsWhileChatting)) {
      State.Save();
    }
...
#pragma region New config menu, needs fixing
    ...
    if (CustomListBoxInt("Configs", &selectedConfigInt, CONFIGS_CHAR), 100 *
    State.dpiScale, ImVec4(0,0,0,0), ImGuiComboFlags_NoArrowButton) {
            isNewConfig = selectedConfigInt == CONFIGS.size() - 2;
            isDelete = selectedConfigInt == CONFIGS.size() - 1;
            if (!isNewConfig && !isDelete) State.selectedConfig =
    CONFIGS[selectedConfigInt]; State.Save(); State.Load();
    }

    if (isNewConfig || isDelete) {
            InputString("Name", &State.selectedConfig);
            if (isNewConfig &&
    (ImGui::Button(CheckConfigExists(State.selectedConfig) ? "Overwrite" :
    "Save"))) { State.Save(); CONFIGS = GetAllConfigs();

                    selectedConfigInt = std::distance(CONFIGS.begin(),
    std::find(CONFIGS.begin(), CONFIGS.end(), State.selectedConfig));
            }
...
#pragma endregion
    ...
    static const std::vector<const char *> DPI_SCALING_LEVEL = {
    ...
    ImGui::SameLine();
    int scaleIndex =
        (int(std::clamp(State.dpiScale, 0.5f, 3.0f) * 100.0f) - 50) / 5;
    if (CustomListBoxInt("Menu Scale", &scaleIndex, DPI_SCALING_LEVEL,
                         100 * State.dpiScale)) {
    ...
    if (!State.GradientMenuTheme) {
      if (ImGui::ColorEdit3("Menu Theme Color", (float *)&State.MenuThemeColor,
      ...
      if (ImGui::ColorEdit3(
              "Gradient Color 1", (float *)&State.MenuGradientColor1,
              ImGuiColorEditFlags__OptionsDefault |
      ...
      ImGui::SameLine();
      if (ImGui::ColorEdit3(
              "Gradient Color 2", (float *)&State.MenuGradientColor2,
              ImGuiColorEditFlags__OptionsDefault |
                  ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_AlphaBar |
                  ImGuiColorEditFlags_AlphaPreview)) {
        State.Save();
    ...
    if (ToggleButton("Match Background with Theme",
                     &State.MatchBackgroundWithTheme)) {
      State.Save();
    }
    ImGui::SameLine();
    ...
    SteppedSliderFloat("Opacity", (float *)&State.MenuThemeColor.w, 0.1f, 1.f,
                       0.01f, "%.2f",
    ...
    if (!(IsHost() || !State.SafeMode)) {
      if (State.userName.length() >= (size_t)13)
        ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f),
                           "Username is too long, gets detected by anticheat. "
                           "This name will be ignored.");
      else if (!IsNameValid(State.userName))
        ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f),
                           "Username contains characters blocked by anticheat. "
                           "This name will be ignored.");
    }

    if ((IsInGame() || IsInLobby()) &&
        (IsNameValid(State.userName) || (IsHost() || !State.SafeMode)) &&
        ImGui::Button("Set Name")) {
      if (IsInGame())
    ...
    if (ToggleButton("Allow other mod users to see you're using",
                     &State.ModDetection)) {
      State.Save();
    }
    ImGui::SameLine();
    ...
    ImGui::Text("Keep safe mode on in official servers (NA, Europe, Asia) to "
                "prevent anticheat detection!");
  }
  if (openSpoofing) {
    ...
    if (State.SpoofGuestAccount) {
      ImGui::SameLine();
      if (ToggleButton("Use Custom Guest Friend Code",
                       &State.UseGuestFriendCode)) {
        State.Save();
      }
      if (State.UseGuestFriendCode) {
        ...
        ImGui::Text("Guest friend code should be <= 10 characters long and "
                    "cannot have a hashtag.");
      }
    ...
    if (State.SpoofLevel) {
      ...
      if (State.SafeMode && (State.FakeLevel <= 0 || State.FakeLevel > 100001))
        ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f),
                           "Level will be detected by anticheat, your level "
                           "will be between 0 and 100001.");
    }
    ...
    // if (ToggleButton("Disable Host Anticheat", &State.DisableHostAnticheat))
    // State.Save();
    if (ToggleButton("Disable Host Anticheat (+25 Mode)",
                     &State.DisableHostAnticheat)) {
