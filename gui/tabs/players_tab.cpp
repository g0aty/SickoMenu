		...
		try {
			if (IsInGame() || IsInLobby()) {
				...
				if (selectedPlayer.has_value()) {
					...
					if (IsInGame() && !selectedPlayer.is_Disconnected() && IsInMultiplayerGame()) ImGui::SameLine();
					if (IsInGame() && !selectedPlayer.is_Disconnected() && IsInMultiplayerGame() && TabGroup("Tasks", openTasks)) {
						CloseOtherGroups(Groups::Tasks);
					}
				}
				...
				if (openTrolling && selectedPlayer.has_value()) {
					...
					if (openTasks && selectedPlayer.has_value() && IsInGame() && !selectedPlayer.is_Disconnected() && IsInMultiplayerGame()) {
			...
			LOG_DEBUG("Error occurred in players tab.");
		}
	}
}