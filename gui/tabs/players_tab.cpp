		...
		if ((IsInGame() || IsInLobby()) && !State.BlinkPlayersTab) {
			...
			if (openPlayer && selectedPlayer.has_value())
			{
				...
				if (IsInGame() && PlayerIsImpostor(GetPlayerData(*Game::pLocalPlayer))
					...
					&& ((*Game::pLocalPlayer)->fields.killTimer <= 0.0f)
					&& !State.InMeeting)
				{
				...
				else if (IsHost() || !State.SafeMode)
				{
					if (IsInGame() && ImGui::Button("Kill"))
					{
						for (PlayerSelection p : selectedPlayers) {
							...
							if (IsInGame())
							{
								bool canMurder = validPlayer.get_PlayerControl()->fields.protectedByGuardianId < 0 || State.BypassAngelProt;
								State.rpcQueue.push(new RpcMurderPlayer(*Game::pLocalPlayer, validPlayer.get_PlayerControl(), canMurder));
							}
							else if (IsInLobby())
							{
								bool canMurder = validPlayer.get_PlayerControl()->fields.protectedByGuardianId < 0 || State.BypassAngelProt;
								State.lobbyRpcQueue.push(new RpcMurderPlayer(*Game::pLocalPlayer, validPlayer.get_PlayerControl(), canMurder));
							}
						}
					}
				}
